...
class Hari_Kemarin extends Controller
{
    // Cache duration
    ...
    public function index()
    {
        ...
        return view('kemarin.index', compact('stats', 'slHarian'));
    }
    ...
    {
        return Cache::remember('kemarin_all_stats', self::CACHE_SHORT, function() {
            // PB IDM
            $pbIdm = DB::selectOne("
                ...
                WHERE tko.TKO_KODESBU = 'I' AND pb.HPBI_CREATE_DT::date = CURRENT_DATE -1
            ")?->total ?? 0;
            ...
            $pbOmi = DB::selectOne("
                ...
                FROM IGRPWT.TBTR_HEADER_PBIDM AS pb
                JOIN IGRPWT.TBMASTER_TOKOIGR AS tko ON pb.HPBI_KODETOKO = tko.TKO_KODEOMI
                WHERE tko.TKO_KODESBU = 'O' AND pb.HPBI_CREATE_DT::date = CURRENT_DATE -1
            ")?->total ?? 0;
    ...
    {
        return Cache::remember('kemarin_sl_harian', self::CACHE_MEDIUM, function() {
            // SL All
            $servicelevelall = DB::select("
                SELECT * FROM (
                    ...
                    WHERE pbo_create_dt::date BETWEEN CURRENT_DATE -1 - INTERVAL '30 days' AND CURRENT_DATE -1
    ...
    {
        $html = Cache::remember('kemarin_pbmentah_idm', self::CACHE_SHORT, function() {
            ...
            $query = "SELECT                count(fdkcab) as toko,
                                        ...
                                        from igrpwt.csv_pb_idm
                                                            ...
                                                            where tgl_proses::date = CURRENT_DATE -1
            ...
            return view('kemarin.partials.pb-mentah-idm', compact('pbmi'))->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_pbmentah_omi', self::CACHE_SHORT, function() {
            ...
            return view('kemarin.partials.pb-mentah-omi', compact('pbmo'))->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_pbsl_idm', self::CACHE_SHORT, function() {
            // Query PB IDM - SAMA PERSIS DENGAN ASLINYA
            $query = "SELECT
                        AA.KDTK AS KODE,
                        ...
                        FROM IGRPWT.TBMASTER_PBOMI WHERE date(PBO_CREATE_DT) = CURRENT_DATE -1
                        ...
                        FROM IGRPWT.TBMASTER_PBOMI WHERE date(PBO_CREATE_DT) = CURRENT_DATE -1 AND PBO_NOKOLI IS NOT NULL
                        AND PBO_KODEOMI IN(SELECT TKO_KODEOMI FROM IGRPWT.TBMASTER_TOKOIGR WHERE TKO_KODESBU = 'I') 
            ...
            $query2 = "SELECT SUBSTRING(pbo_pluigr FROM 1 FOR 6) || '0' AS plu,
                ...
                FROM (
                    ...
                    FROM igrpwt.tbmaster_pbomi
                    WHERE DATE(PBO_CREATE_DT) = CURRENT_DATE -1
                    AND pbo_kodesbu = 'I'
                    AND PBO_CREATE_BY NOT IN ('INS','TSW')
            ...
            return view('kemarin.partials.pb-sl-idm', compact('pbi', 'sli'))->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_pbsl_omi', self::CACHE_SHORT, function() {
            // Query PB OMI - SAMA PERSIS DENGAN ASLINYA
            $query = "SELECT
                        AA.KDTK AS KODE,
                        TKO_NAMAOMI AS NAMA_TOKO,
            ...
            return view('kemarin.partials.pb-sl-omi', compact('pbo', 'slo'))->render();
        });
    ...
    {
        Cache::forget('kemarin_pbmentah_idm');
    ...
    {
        Cache::forget('kemarin_pbmentah_omi');
    ...
    {
        Cache::forget('kemarin_all_stats');
    ...
    {
        Cache::forget('kemarin_sl_harian');
        $slHarian = $this->getCachedSLHarian();
        
        return response()->json([
            'html_all' => view('kemarin.partials.sl-harian', ['data' => $slHarian['all'] ?? [], 'title' => 'ALL'])->render(),
            'html_idm' => view('kemarin.partials.sl-harian', ['data' => $slHarian['idm'] ?? [], 'title' => 'IDM'])->render(),
            'html_omi' => view('kemarin.partials.sl-harian', ['data' => $slHarian['omi'] ?? [], 'title' => 'OMI'])->render(),
        ]);
    }
    ...
    {
        $html = Cache::remember('kemarin_rupiah_all', self::CACHE_SHORT, function() {
            $data = DB::select("WITH
                                        -- Ambil data PB OMI
                                        pbomi_raw AS (
                                            SELECT 
                                            ...
                                            FROM IGRPWT.tbmaster_pbomi
                                            WHERE DATE_TRUNC('day', pbo_create_dt) = CURRENT_DATE -1 
                                        ...
                                        ORDER BY 
                                                ...
                                                zona,
                                                jalur");
            return view('kemarin.partials.rupiah-table', ['data' => $data, 'type' => 'all'])->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_rupiah_idm', self::CACHE_SHORT, function() {
            $data = DB::select("WITH
                                        -- Ambil data PB OMI
                                        pbomi_raw AS (
                                            SELECT 
                                        ...
                                        ORDER BY 
                                                ...
                                                zona,
                                                jalur");
            return view('kemarin.partials.rupiah-table', ['data' => $data, 'type' => 'idm'])->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_rupiah_omi', self::CACHE_SHORT, function() {
            $data = DB::select("WITH
                                        -- Ambil data PB OMI
                                        pbomi_raw AS (
                                            SELECT 
                                        ...
                                        ORDER BY 
                                                ...
                                                zona,
                                                jalur");
            return view('kemarin.partials.rupiah-table', ['data' => $data, 'type' => 'omi'])->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_monitor_picking', self::CACHE_SHORT, function() {
            $monitorPicking = DB::select("SELECT  
                                            ...
                                            FROM (
                                                ...
                                                WHERE pia_tglpick::date = CURRENT_DATE -1
                                            ...
                                            LEFT JOIN (
                                                ...
                                                WHERE dca_tgl::date = CURRENT_DATE -1
                                            ...
                                            WHERE hdp_create_dt::date = CURRENT_DATE -1
                                            HAVING COUNT(hdp_flag) > 0
                                            ORDER BY zona, jalur) AB
            ");
            return view('kemarin.partials.picking-table', compact('monitorPicking'))->render();
        });
    ...
    {
        $html = Cache::remember('kemarin_monitor_loading', self::CACHE_SHORT, function() {
            $monitorLoading = DB::select("WITH base_data_idm AS (
                                                            ...
                                                            WHERE DATE(hpbi_create_dt) = CURRENT_DATE -1 
                                                        ),
                                                        summary_ab_idm AS (
                                                            SELECT 
                                                                tgl,
                                                        ...
                                                        base_data_pot AS (
                                                            SELECT 
                                                                ...
                                                                'POT' AS tipe_toko
                                                            FROM igrpwt.tbtr_header_pot
                                                            WHERE DATE(hdp_create_dt) = CURRENT_DATE -1 
                                                        ),
                                                        summary_ab_pot AS (
                                                            SELECT 
                                                                tgl,
                                                        ...
                                                        LEFT JOIN summary_bb_pot bb 
                                                            ON ab.tgl = bb.tgl AND ab.tipe_toko = bb.tipe_toko
            ");
            return view('kemarin.partials.loading-table', compact('monitorLoading'))->render();
        });
