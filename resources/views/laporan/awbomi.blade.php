...
function formatRangeTanggal(tglMulai, tglSelesai) {
    if (!tglSelesai) {
        return formatTanggalExcel(tglMulai);
    }
    return `${formatTanggalExcel(tglMulai)} s/d ${formatTanggalExcel(tglSelesai)}`;
}
...
$(document).ready(function () {
    ...
    $("#tanggal_mulai_picker").on("change.datetimepicker", function (e) {
        if (e.date) {
            // Atur batas minimum tanggal selesai TANPA mengubah nilainya
            $('#tanggal_selesai_picker').datetimepicker('minDate', e.date.clone().startOf('day'));

            // Hapus tanggal selesai jika sudah terisi tapi lebih kecil dari tanggal mulai
            let tglSelesaiVal = $('#tanggal_selesai_input').val();
            if (tglSelesaiVal && moment(tglSelesaiVal, 'DD-MM-YYYY').isBefore(e.date, 'day')) {
                $('#tanggal_selesai_input').val('');
            }
        } else {
    ...
    $("#tanggal_selesai_picker").on("change.datetimepicker", function (e) {
        if (e.date) {
            // Atur batas maksimum tanggal mulai TANPA mengubah nilainya
            $('#tanggal_mulai_picker').datetimepicker('maxDate', e.date.clone().endOf('day'));

            // Hapus tanggal mulai jika sudah terisi tapi lebih besar dari tanggal selesai
            let tglMulaiVal = $('#tanggal_mulai_input').val();
            if (tglMulaiVal && moment(tglMulaiVal, 'DD-MM-YYYY').isAfter(e.date, 'day')) {
    ...
    $('#submit_form').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let tglExcel = formatRangeTanggal(tanggal_mulai, tanggal_selesai);
                ...
                var table = $('#example1').DataTable({
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            title: 'AWB IPP OMI ' + tglExcel,
                            filename: 'AWB_IPP_OMI_' + tglExcel,
