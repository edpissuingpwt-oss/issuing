...
@section('title', 'LAPORAN REKAP NPB/NPT')
@section('content')
@push('styles')
    ...
    <div class="container-fluid">
        ...
        <div class="card card-success">
            ...
            <div class="card-header small-box-issuing-header text-white d-flex justify-content-between align-items-center">
                <h3 class="card-title">REKAP NPB/NPT</h3>
            </div>
            <form action="" method="post">
                <div class="card-body modern-body">
                    ...
                    <div class="picker-cards">
                        ...
                        <input type="radio" name="r3" id="radioSuccess2" value="npb" checked>
                        ...
                        <!-- Pot -->
                        <input type="radio" name="r3" id="radioSuccess1" value="npt">
...
function loadTable(status, tanggal_mulai, tanggal_selesai, autoExport = false) {
    $.ajax({
        url: "{{ route('rekapnpb.table') }}",
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            flag: status,
        ...
        success: function(data) {
            ...
            let [dayMulai, monthMulai, yearMulai] = tanggal_mulai.split('-');
            let [daySelesai, monthSelesai, yearSelesai] = tanggal_selesai.split('-');

            // susun ulang jadi YYYY_MM-DD
            let tglFormat = `${yearMulai}_${monthMulai}-${dayMulai}_sampai_${yearSelesai}_${monthSelesai}-${daySelesai}`;

            let filenameCsv = 'REKAP_' + statusUpper + '_' + plant + '_' + tglFormat;
...
document.getElementById('submit_form').addEventListener('click', function() {
    ...
    if (status && tanggal_mulai && tanggal_selesai) {
        loadTable(status, tanggal_mulai, tanggal_selesai);
    } else {
...
$('#export_excel_form').on('submit', function (e) {
    ...
    $('#export_selesai').val(tanggal_selesai);
});
