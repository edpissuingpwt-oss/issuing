@extends('layouts.app')
@section('title', 'LAPORAN SARANA PICKING')
@section('content')
@push('styles')
...
input#idm:checked + .picker-card,
input#omi:checked + .picker-card {
    background: linear-gradient(135deg, #007bff, #00c6ff);
    color: white;
    box-shadow: 0 6px 15px rgba(0, 123, 255, 0.4);
    border-color: #007bff;
}
input#idm:not(:checked) + .picker-card,
input#omi:not(:checked) + .picker-card {
    background: #f1f6ff;
    color: #007bff;
}
.picker-card:hover {
          ...
          <!-- Picker Jenis -->
                <div class="mb-4">
                    ...
                    <div class="picker-cards">
                        ...
                        <!-- IDM -->
                        <input type="radio" name="flag" id="idm" value="idm" checked>
                        <label for="idm" class="picker-card">
                            <h5>
                        ...
                        <!-- Pot -->
                        <input type="radio" name="flag" id="omi" value="omi">
                        <label for="omi" class="picker-card">
                            <h5>
            ...
            <div class="mb-4">
                <label for="tanggal_mulai_input" class="form-label fw-semibold">Bulan :</label>
...
<script>
    function getSarana() {
        const val = $('input[name="flag"]:checked').val(); // idm / omi
        return val ? val.toUpperCase() : '';
    }

    function formatBulanMMMYYYY(val) {
        if (!val) return '';
    ...
    $(document).ready(function () {
        ...
        $('#submit_form').on('click', function () {
            let bulan = $('#tanggal_mulai_input').val(); // format: YYYY-MM
            ...
            // Pastikan deklarasi dan penggunaan variabel berada dalam blok kode yang sama
            const startDateFormatted = moment(bulan, 'YYYY-MM').startOf('month').format('YYYY-MM-DD');
            const endDateFormatted = moment(bulan, 'YYYY-MM').endOf('month').format('YYYY-MM-DD');
            let rawBulan = $('#tanggal_mulai_input').val(); // 2026-02
            ...
            let bulanMMMYYYY = formatBulanMMMYYYY(rawBulan);
            // Kirim data via AJAX dengan parameter baru
            $.ajax({
                url: "{{ route('sarana.fetch') }}",
                type: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    flag: flag,
                    start_date: startDateFormatted, // Kirim 2025-08-01
                    end_date: endDateFormatted // Kirim 2025-08-31
                },
                    ...
                    var table = $('#example1').DataTable({
                        ...
                        buttons: [
                            ...
                            },
                            {
                                ...
                                title: function () {
                                    return 'SARANA ' + getSarana() + '_' + formatBulanMMMYYYY(bulan);
                                },
                                filename: function () {
                                    return 'SARANA_' + getSarana() + '_' + formatBulanMMMYYYY(bulan);
                                }
                            }
                        ],
                        scrollX: true,
