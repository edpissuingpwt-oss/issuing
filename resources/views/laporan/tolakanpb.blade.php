@extends('layouts.app')
@section('title', 'LAPORAN TOLAKAN PB')
...
<div class="container-fluid">
    ...
    <form id="form_slpot">
        <div class="card-body modern-body">
            ...
            <!-- Keterangan, Jenis Laporan, PLU IGR -->
            <div class="row mb-4">
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-4">
                        <div class="card-header bg-light header-bold text-primary text-uppercase text-center">
                            Keterangan Tolakan
                        </div>
                        <div class="card-body">
                            <select class="form-control form-select" name="kodeTolakan" id="kodeTolakan">
                                <option value="All">All</option>
                                @foreach($tolakanidm as $row)
                                    <option value="{{ $row->tlko_kettolakan }}">{{ $row->tlko_kettolakan }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-4">
                        ...
                        <div class="card-body">
                            <select class="form-control form-select" name="jenisLaporan" id="jenisLaporan">
                                <option value="1">1. Laporan per Produk</option>
                                <option value="2">2. Laporan per Produk per Toko</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="row justify-content-center mb-4">
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm rounded-4" id="pluigrCard">
                        ...
                        <div class="card-body text-center">
                            <input 
                                ...
                                id="pluigr"
                                name="pluigr"
                                placeholder="Contoh: 0060410"
                                disabled
                            >

                            <small class="text-muted mt-2 d-block">
                                Aktif hanya untuk Jenis Laporan per Produk per Toko
                            </small>
                        </div>
...
$(function () {
    ...
    // ======================================
    // Toggle PLU IGR berdasarkan jenis laporan
    // ======================================
    function togglePluigr() {
        ...
        if (jenis === '2') {
            $('#pluigr')
                .prop('disabled', false)
                .closest('#pluigrCard')
                .removeClass('plu-disabled');
        } else {
            $('#pluigr')
                .prop('disabled', true)
                .val('')
                .closest('#pluigrCard')
                .addClass('plu-disabled');
        }
    }
    ...
    var endWrapper   = $('#reservationdate2');
    var startInput   = $('#tanggal_mulai');
    var endInput     = $('#tanggal_selesai');
    ...
    try {
        if (endWrapper && endWrapper.data('DateTimePicker')) {
            endWrapper.datetimepicker('destroy');
            console.log('Destroyed existing endPicker (DateTimePicker)');
        }
    } catch(e){ /* ignore */ }
    ...
    try {
        endWrapper.datetimepicker($.extend({}, commonOpts, { useCurrent: false }));
    } catch (e) {
        console.error('Gagal inisialisasi endPicker:', e);
    }
    ...
    endWrapper.on('show.datetimepicker shown.datetimepicker datetimepicker.show dp.show', function(){
        bumpZindex();
    });
    ...
    endWrapper.on('change.datetimepicker dp.change', onEndChange);
    ...
    endInput.on('focus click', function () {
        try {
            endWrapper.datetimepicker('show');
        } catch(e){
            endWrapper.find('.input-group-append').trigger('click');
        }
    });
    ...
    endInput.on('blur', function(){
        var v = $(this).val();
        if (!v) {
            try { endWrapper.datetimepicker('clear'); } catch(e){}
            return;
        }
        var m = moment(v, 'DD-MM-YYYY', true);
        if (m.isValid()) {
            try { endWrapper.datetimepicker('date', m); } catch(e){
                try { endWrapper.data('DateTimePicker') && endWrapper.data('DateTimePicker').date(m); } catch(e2){}
            }
        } else {
            console.warn('Tanggal selesai tidak valid:', v);
        }
    });
    ...
    $('#submit_form').on('click', function () {
        let jenis = $('#jenisLaporan').val();
        let keterangan = $('#kodeTolakan').val();
        let tanggal_mulai = $('#tanggal_mulai').val();
        let tanggal_selesai = $('#tanggal_selesai').val();
        let jenis_toko = $('input[name="r3"]:checked').val();
        let pluigr = $('pluigr').val();
