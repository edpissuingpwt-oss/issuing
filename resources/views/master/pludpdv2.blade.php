@extends('layouts.app')
@section('title', 'MASTER ALAMAT RAK DPD V2')
        ...
        <div class="card-header modern-header d-flex justify-content-between align-items-center">
            ...
            <div class="card-body">
                    <div class="table-responsive">
                        <table id="pludpd" class="table table-striped table-bordered table-sm" style="min-width: 1000px;">
                            <thead class="bg-gradient-info text-white">
                                <tr>
                                    <th rowspan="2" style="text-align: center;">No</th>
                                    <th colspan="9" style="text-align: center;">Produk</th>
                                    <th rowspan="2" style="text-align: center;">FLAG</th>
                                    <th colspan="3" style="text-align: center;">TAG</th>
                                    <th rowspan="2" style="text-align: center;">PERIODE AWAL</th>
                                    <th rowspan="2" style="text-align: center;">PERIODE AKHIR</th>
                                    <th rowspan="2" style="text-align: center;">TGL DISCONTINUE</th>
                                    <th rowspan="2" style="text-align: center;">ACOST</th>
                                    <th rowspan="2" style="text-align: center;">LPP</th>
                                    <th rowspan="2" style="text-align: center;">RP LPP</th>
                            ...
                            @foreach ($results as $index => $row)
                                <tr>
                                    ...
                                    <td class="text-center">{{ $row->kat }}</td>
                                    <td class="text-center">{{ $row->plu_igr }}</td>
                                    <td class="text-center">{{ $row->plu_idm }}</td>
                                    <td class="text-center">{{ $row->plu_omi }}</td>
                                    <td class="text-center">{{ $row->desk }}</td>
                                    <td class="text-center">{{ $row->unit }}</td>
                                    <td class="text-center">{{ $row->frac }}</td>
                                    <td class="text-center">{{ $row->flag }}</td>
                                    <td class="text-center">{{ $row->tag_igr }}</td>
                                    <td class="text-center">{{ $row->tag_idm }}</td>
                                    <td class="text-center">{{ $row->tag_omi }}</td>
                                    <td class="text-center">{{ $row->retur_idm_pertama }}</td>
                                    <td class="text-center">{{ $row->retur_idm_terakhir }}</td>
                                    <td class="text-center">{{ $row->tgldiscontinue }}</td>
                                    <td class="text-right">{{ number_format($row->acost) }}</td>
                                    <td class="text-center">{{ $row->lpp }}</td>
                                    <td class="text-right">{{ number_format($row->rp_lpp) }}</td>
    ...
    var table = $('#pludpd').DataTable({
        ...
        buttons: [
            ...
            },
            {
                ...
                title: 'ALAMAT RAK DPD',
                filename: 'ALAMAT_RAK_DPD',
            }
        ],
        scrollX: true,
        ordering: false,
