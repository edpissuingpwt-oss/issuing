@extends('layouts.app') 
@section('title', 'ITEM & STATUS BARCODE KOLI')
@section('content')
@push('styles')
...
input#barcodekoli:checked + .picker-card,
input#isikoli:checked + .picker-card {
    background: linear-gradient(135deg, #007bff, #00c6ff);
    color: white;
    box-shadow: 0 6px 15px rgba(0, 123, 255, 0.4);
    border-color: #007bff;
}
input#barcodekoli:not(:checked) + .picker-card,
input#isikoli:not(:checked) + .picker-card {
    background: #f1f6ff;
    color: #007bff;
}
.picker-card:hover {
...
[data-theme="dark"] input#barcodekoli:not(:checked) + .picker-card,
[data-theme="dark"] input#isikoli:not(:checked) + .picker-card {
    background: #1f2a3a;
    color: #6cb4ff;
}
    ...
    <div class="container-fluid">
        ...
        <div class="card card-success">
            ...
            <div class="card-header small-box-issuing-header text-white d-flex justify-content-between align-items-center">
                <h3 class="card-title">ITEM & STATUS BARCODE KOLI</h3>
            </div>
            <form action="" method="post">
                ...
                <div class="mb-4">
                    ...
                    <div class="picker-cards">
                        ...
                        <!-- Item -->
                        <input type="radio" name="flag" id="isikoli" value="isikoli">
                        <label for="isikoli" class="picker-card">
                            <h5>
                        ...
                        <!-- Status -->
                        <input type="radio" name="flag" id="barcodekoli" value="barcodekoli">
                        <label for="barcodekoli" class="picker-card">
                            <h5>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <label for="noPick">No Pick :</label>
                            <input type="text" id="noPick" name="noPick" class="form-control" placeholder="No Picking">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <label for="noPick">No Barcode :</label>
                            <input type="text" id="noKoli" name="noKoli" class="form-control" placeholder="No Barcode Koli">
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    $(document).on('change', 'input[name="flag"]', function () {
        ...
        let flag = $(this).val();
        if (flag === 'barcodekoli') {
            $('#noKoli').prop('disabled', true).val('');
        } else {
            $('#noKoli').prop('disabled', false);
        }
    });

    document.getElementById('submit_form').addEventListener('click', function () {
        let flag = $('input[name="flag"]:checked').val();
        let noPick = $('#noPick').val();
        let noKoli = $('#noKoli').val();

        if (!flag) {
            alert('Harap pilih item atau status!');
            return;
        }

        if ((flag !== 'isikoli') && (!noPick)) {
            alert('Harap isi semua kolom');
            return;
        }

        // validasi khusus untuk status yg butuh tanggal
        if ((flag !== 'barcodekoli') && (!noKoli) && (!noPick)) {
            alert('Harap isi semua kolom');
            return;
        }

        $.ajax({
            url: "{{ route('koli.table') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                flag: flag,
            ...
            success: function (data) {
                ...
                let selectedflag = $('input[name="flag"]:checked').attr('id');
                let flagText = $('label[for="' + selectedflag + '"] h6').text().trim();
                ...
                $("#example1").DataTable({
                    ...
                    "buttons": [
                        ...
                        },
                        {
                            ...
                            title: flagText + " " + todayStr,
                            filename: flagText.replace(/\s+/g, '_') + "_" + todayStr
                        },
                        {
                            extend: 'print',
                            ...
                            customize: function (win) {
                                // Tambahkan CSS khusus untuk thermal printer
                                $(win.document.body).css('font-size', '10px');

                                // Atur ukuran halaman thermal (58mm x 297mm)
                                $(win.document.head).append(
                                    '<style>@page { size: 58mm 297mm; margin: 2mm; } body { margin:0; }</style>'
                                );

                                // Tambahkan styling tabel
    ...
    document.getElementById('clear_form').addEventListener('click', function () {
        ...
        $('.flag-option').removeClass('active'); // reset card
        $('#noKoli').prop('disabled', false).val('');
    });
</script>
