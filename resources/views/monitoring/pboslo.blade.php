...
<div class="container-fluid">
    <div class="card modern-card shadow-lg border-0">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="pbomi" class="table table-bordered table-striped" style="width:100%">
                    ...
                    @php
                        $totalItemP = array_sum($rphData[0]);
                        $totalItemR = array_sum($rphData[1]);
                        $totalQtyP  = array_sum($rphData[2]);
                        $totalQtyR  = array_sum($rphData[3]);
                        $totalRphP  = array_sum($rphData[4]);
                        $totalRphR  = array_sum($rphData[5]);
                    @endphp

                    <tfoot>
                        <tr>
                            <th colspan="3" class="text-center">TOTAL</th>
                            <th class="text-right">{{ number_format($totalItemP) }}</th>
                            <th class="text-right">{{ number_format($totalItemR) }}</th>
                            <th class="text-right">
                                {{ ($totalItemP ?? 0) > 0 ? round(($totalItemR ?? 0) / $totalItemP * 100) : 0 }} %
                            </th>
                            <th class="text-right">{{ number_format($totalQtyP) }}</th>
                            <th class="text-right">{{ number_format($totalQtyR) }}</th>
                            <th class="text-right">
                                {{ ($totalQtyP ?? 0) > 0 ? round(($totalQtyR ?? 0) / $totalQtyP * 100) : 0 }} %
                            </th>
                            <th class="text-right">{{ number_format($totalRphP) }}</th>
                            <th class="text-right">{{ number_format($totalRphR) }}</th>
                            <th class="text-right">{{ $totalRphP != 0 ? round($totalRphR / $totalRphP * 100) : 0 }}%</th>
                        </tr>
                    </tfoot>
                </table>
    ...
    <div class="card mt-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="tokoomi" class="table table-bordered table-striped" style="width:100%">
                    <thead class="bg-gradient-info text-white">
                        <tr>
                            <th>No</th>
                            <th>PLU</th>
                    ...
                    @foreach($slo as $index => $row)
                        <tr>
                            ...
                            <td>{{ $row->plu }}</td>
                            <td>{{ $row->deskripsi }}</td>
                            <td>{{ number_format(round($row->jml_toko), 0, ".", ",") }}</td>
                            <td>{{ number_format(round($row->qtyo), 0, ".", ",") }}</td>
                            <td>{{ number_format(round($row->qtyr), 0, ".", ",") }}</td>
                            <td>{{ number_format(round($row->slqty), 0, ".", ",") }}%</td>
                            <td>{{ number_format(round($row->nilaio), 0, ".", ",") }}</td>
                            <td>{{ number_format(round($row->ttlnilai), 0, ".", ",") }}</td>
                            <td>{{ number_format(round($row->slnilai), 0, ".", ",") }}%</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
...
$(document).ready(function() {
    ...
    var table1 = $('#tokoomi').DataTable({
        ...
        buttons: [
            ...
            },
            {
                ...
                title: 'SERVICE LEVEL OMI ' + today,
                filename: 'SERVICE_LEVEL_OMI_' + today,
    ...
    // Tabel 2: PB OMI TODAY
    var table2 = $('#pbomi').DataTable({
        dom: '<"d-flex justify-content-between align-items-center mb-2"lfB>rtip',
        buttons: [
            {
                extend: 'copy',
